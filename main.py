...
class ReferralClient:
    ...
    def _load_credentials(self) -> bool:
        try:
            ...
            log_step("Each line in reg.txt must have: email,password,username", "error")
            return False
    ...
    def _update_proxy(self):
        if self.proxy_manager:
            ...
            if self.current_proxy:
                ...
                log_step("No valid proxy retrieved from ProxyManager.", "warning")
        else:
            self.current_proxy = None
    ...
    async def _make_request(
        ...
        log_step(f"Making {method.upper()} request to {url}", "info")
        log_step(f"Request headers: {headers}", "info")
        log_step(f"Request payload: {json_data}", "info")
        if proxies:
            log_step(f"Using proxies: {proxies}", "info")
        ...
        try:
            ...
            try:
                ...
                log_step(f"JSON parsed successfully: {response_json}", "info")
            except Exception as e:
                log_step(f"Failed to parse JSON response: {str(e)}", "error")
                return {"success": False, "msg": f"Failed to parse JSON: {str(e)}"}
            return response_json

        except Exception as e:
            log_step(f"Request exception: {str(e)}", "error")
    ...
    async def login(self, captcha_service) -> str:
        try:
            ...
            log_step("Captcha token obtained for login", "success")
    ...
    async def process_referral(self, ref_code: str, captcha_service, referral_number: int):
        try:
            if not self._load_credentials():
                return None

            log_step(f"Loaded credentials for: {self.email}", "info")
            log_step("Getting captcha token for registration...", "info")
            reg_captcha_token = await captcha_service.get_captcha_token_async()
            log_step("Captcha token obtained for registration", "success")
            ...
            register_data = {
                ...
                'recaptcha_token': reg_captcha_token
            }
...
async def main():
    # SAMPLE REFER CODES YOU CAN REPLACE THE REFER CODE WITH YOURS HERE
    referral_codes = [
        "mjIpmj2MWwKKyjz",
        "FFbRTqtmtKos6Dr",
        "28Z4fXIg2KkGGAn",
    ...
    api_key = "REPLACE_HERE_YOUR_API_KEY"
    try:
        with open('reg.txt', 'r') as f_reg:
            credentials_list = [line.strip() for line in f_reg if line.strip()]
    except FileNotFoundError:
        log_step("File reg.txt not found. Exiting.", "error")
        return

    num_referrals = len(credentials_list)
    if num_referrals == 0:
        log_step("No credentials found in reg.txt. Exiting.", "error")
        return
    use_proxies = True
    proxy_manager = None
    if use_proxies:
    ...
    for i in range(num_referrals):
        ...
        tasks = [
            client.process_referral(ref_code, captcha_service, referral_number=i + 1)
        ]
        ...
        if i < num_referrals - 1:
            ...
            log_step(f"Waiting {delay:.2f} seconds before next referral...", "info")
